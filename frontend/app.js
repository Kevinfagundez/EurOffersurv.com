...
function initLogoutModal() {
    ...
    // Si aún no existe el modal en el HTML, no hacemos nada
...
async function initDashboard() {
    const allowed = await (auth.requireAuth ? auth.requireAuth('/frontend/index.html') : Promise.resolve(true));
    if (!allowed) return;
    ...
    // ✅ Retirar (placeholder pro)
    initWithdraw(user);

    // ✅ Logout modal
    initLogoutModal();

    // Inicializar TheoremReach al final
...
async function resolveCurrentUser() {
    ...
    if (auth.checkSession) {
        try {
            ...
            if (s && (s.user || s.data?.user)) return normalizeUser(s.user || s.data.user);
        } catch (e) {
            console.warn('auth.checkSession falló:', e);
        }
    }

    try {
        const res = await fetch('/backend/api/user-data.php', { credentials: 'include' });
        if (!res.ok) return null;
        const data = await res.json();
        if (data && data.user) return normalizeUser(data.user);
    } catch (e) {
        console.warn('Fetch /backend/api/user-data.php falló:', e);
    }
...
function initNotifications(user) {
    ...
    // DEMO: si no hay, cargamos algunas (por usuario)
    ...
    function render() {
        ...
        // Orden: no leídas arriba, más nuevas primero
    ...
    return [
        {
            ...
            action: { type: 'link', href: '/frontend/dashboard.html' }
        },
        {
            id: cryptoId(),
            title: 'Consejo',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        if (auth.redirectIfAuthenticated) auth.redirectIfAuthenticated('/frontend/dashboard.html');
